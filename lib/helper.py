...
async def optimize(args, funcs=None):
    """
        回调函数处理
    """
    if funcs is None:
...
def handle_expect_salary(args=None, *extra):
    ...
    if "expect_salary" in args:
        ...
        elif '薪' in salary and (
                matches := re.findall(r'(\d+\.\d+)万-(\d+\.\d+)万（(\d+)K/月-(\d+)K/月\*(\d+)薪）', salary, re.I | re.S)):
...
def handle_position_language(args=None, *extra):
    """
        职位语言要求
    """
